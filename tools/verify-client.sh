...
flutter test. --fix additionally runs dart fix --apply and dart format lib test
between pub get and analysis. Review the resulting git diff before committing.
...
printf '\nAll Flutter verification steps passed. Logs: %s\n' "$log_dir"
if [[ "$fix" == true ]]; then
